...
SERVICE_NAME = "HenkerDPI V2"
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
FONT = "Helvetica"
LOCK_FILE = os.path.join(SCRIPT_DIR, ".henkerdpi_v2.lock")
...
class HenkerDPIApp(ctk.CTk):
    def __init__(self):
        ...
        self._icon_png = icon_path
        # macOS icns kullanir, png fallback
    ...
    def _build_ui(self):
        ...
        for idx, (lbl_key, val, clr) in enumerate([
            ...
            ("RST", "pf drop", info_colors[2]),
            ("QUIC", "pf drop", info_colors[3]),
        ]):
    ...
    def _set_mode(self, mode):
        ...
        # Kategorileri goster/gizle
        if mode == MODE_ALL:
            self._cat_card.pack_forget()
        else:
            # mode_card'dan sonra pack et
            self._cat_card.pack(fill="x", pady=(6, 0),
                                after=self._mode_card)

        # Engine calisiyorsa ayarlari guncelle
        if self._engine:
    ...
    def _check_autostart(self):
        plist_path = os.path.expanduser(
            f"~/Library/LaunchAgents/com.henkerr.henkerdpi.v2.plist")
        self._auto_var.set(os.path.exists(plist_path))
...
def main():
    # Single instance check (fcntl file lock)
